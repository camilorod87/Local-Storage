...
{
	listadoPersonas = [];//definimos esta variable como publica
		...
		for(var i = 0; i < listadoPersonas.length; i++)
		{
			...
			if(muestra)
			{
				...
				nom_div("e_" + i).addEventListener('click', function(event)
				{
					...
					//pasamos los datos guaradados a las cajas de texto para ser editados
				    ...
				    listadoPersonas.splice(ind,1);//se borra del local storage para no duplicar los datos
                    confirm("¿AHORA PUEDE MODIFICAR LOS DATOS..");
				...
				nom_div("d_" + i).addEventListener('click', function(event)
				{
					var ind = event.target.id.split("_")[1];
					var idUser = listadoPersonas[ind].identificacion;
					if(confirm("¿ESTA SEGUR@ DE COMPLETAR ESTA ACCION?"))
					{
						ind = buscaIndice(idUser);                        
                        listadoPersonas.splice(ind,1);
                        imprimeUsuarios();
                        confirm("REGISTRO BORRADO EXITOSAMENTE.")
                     }
					}
				);
			}
